...
export const DocumentReviewPage: React.FC = () => {
  ...
  const transactionType = String(document.candidate_transaction.proposed_transaction_type ?? '');
  ...
  const counterparties = customerTypes.has(transactionType)
    ? (customers.data ?? [])
  ...
  const lookupError = projects.isError || customers.isError || vendors.isError;
  return <div className="grid gap-6 lg:grid-cols-2">
    ...
    <DocumentReviewForm
      document={document}
      projects={(projects.data ?? []).filter(({ project_status }) => ['PLANNED', 'ACTIVE', 'ON_HOLD'].includes(project_status))}
      counterparties={counterparties}
      lookupsLoading={projects.isLoading || customers.isLoading || vendors.isLoading}
      lookupError={lookupError ? 'Daftar proyek atau pihak transaksi tidak dapat dimuat. Muat ulang halaman sebelum menyimpan.' : undefined}
