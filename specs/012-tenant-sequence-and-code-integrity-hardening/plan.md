...
- The Alembic chain reaches `023_historical_seq_bootstrap`, and `uv run alembic check` reports no new upgrade operations. The final migration chain is drift-free.
...
- Tracked PostgreSQL regression tests and production clean-transaction retry behavior are implemented and verified in CP1-CP6.
...
- Baseline: fresh Alembic upgrade applied all revisions through `023_historical_seq_bootstrap`; current database also reports that head.
...
- Baseline verification: model registration is complete, `023_historical_seq_bootstrap` is the current migration head, and `uv run alembic check` reports `No new upgrade operations detected.`
- Retry implementation checkpoint: PostgreSQL poisoned-session behavior and the production bounded clean-transaction retry boundary are verified; retries establish clean state and do not duplicate authoritative effects.
...
- Add the Feature 012 forward Alembic revisions after current head `021_fixed_asset_enhancements`; the final head is `023_historical_seq_bootstrap`, and live schema/constraint names are verified.
...
| Duplicate prevention | PASS | PostgreSQL allocator and generator tests produce unique committed values under concurrency; retry and database constraints remain defense-in-depth. |
...
| Audit trail | PASS | Retry paths preserve actor propagation and CP6 verifies at-most-once audit effects. |
...
| Financial invariants | PASS | PostgreSQL rollback/retry graph tests verify balanced, at-most-once financial effects; posting rules remain unchanged. |
| Protected storage | PASS | Protected storage paths are excluded and must not be accessed. |

| Gate status: PostgreSQL concurrency, constraint, preflight, first-row, rollback, counter-design, historical-bootstrap, and current Alembic-baseline evidence is verified. Production allocator, generator migrations, bounded clean retry, and tracked PostgreSQL regressions are complete and green. |
...
After preflight duplicate checks and live constraint-name verification, add forward Alembic revisions after `021_fixed_asset_enhancements`: revision 022 creates `tenant_sequences` and changes only the three confirmed mismatches to `(organization_id, code)` constraints; revision 023 seeds sequences monotonically from historical codes. Update SQLAlchemy models to match. Verify upgrade and downgrade on a disposable PostgreSQL database; fail closed if expected constraints are absent or duplicate tuples exist.
