...
- [ ] **TENANT-R07**: Tenant A cannot reconcile a bank statement line against Tenant B `Transaction` via `BankReconciliation.transaction_id`.
- [ ] **TENANT-R08**: Foreign-tenant and nonexistent foreign references fail closed uniformly with HTTP 404 `EntityNotFoundException` without existence, owner, or metadata leakage.
- [ ] **TENANT-R09**: Valid same-tenant references continue to work seamlessly across all 7 fields (positive controls).
- [ ] **TENANT-R10**: Existing RBAC matrices from AUTHZ-001 remain 100% intact (`VIEWER` denied with 403; `ADMIN`, `MANAGER`, `OPERATOR` authorized at route perimeter).
